...
@interface MasterTaskVC ()<UITableViewDelegate, UITableViewDataSource>
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"大师任务";
    self.pageNo = 1;
    [self drawView];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.masterTaskModel = self.dataArray[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    self.masterTaskModel = self.dataArray[indexPath.row];
     vc.taskId =self.masterTaskModel.taskId;
...
- (void)requestData {
    ...
    [params setValue:@(2) forKey:@"taskMode"];//任务模块 1-悬赏任务 2-大师任务
    [params setValue:@(self.pageNo) forKey:@"pageNo"];
    [params setValue:@(10) forKey:@"pageSize"];
    
    [CBConnect getUserCenterListMyTasks:params success:^(id responseObject) {
